                ...
                <div class="row border-bottom bg-white" >
                    ...
                    <div class="col-md-2 col-12 d-flex align-items-center justify-content-center bg-white m-md-0 m-2 ">
                        ...
                        @if ($order->surat === null)
                            ...
                            {{-- jika pending --}}
                            @if ($order->items->every(function ($item) {return $item->status === 'Pending';}))
                            ...
                            {{-- jika di-reject --}}
                            @if ($order->items->every(function ($item) {return $item->status === 'Rejected';}))
                            @endif

                        {{-- else, show button view surat (SPJ) --}}
                        @else
                            <a href="{{ asset('storage/posts/surat/' . $order->surat) }}" class="btn btn-primary w-75" target="_blank" view>Lihat PDF</a>
                        @endif
